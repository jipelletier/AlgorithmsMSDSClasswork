...
# In terms of  𝑘,𝑛, the worst case complexity is  Θ(𝑘+log(𝑛)) for each insertion operation.
...
# Overall complexity =  Θ(𝑘+log(𝑛)) in the worst case.
...
class MinHeap:
    ...
    def insert(self, elt):
        ...
        # get the index of new elt
        index = len(self.H)-1
        
        if(index > 1):
            parent_index = index//2
            
            # compare new elt with parent and swap if needed
...
class TopKHeap:
    ...
    def insert(self, elt):
        ...
        # check where elt belongs in the array
